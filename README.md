...
Telegram (voice/text/forwarded note)
  ...
  -> waitUntil(process):
      1. Ingest     download voice via getFile -> transcribe (Gemini, audio input) -> store fragment -> discard audio
      ...
      2b. News      best-effort Google News RSS lookup for relevant recent context;
...
3. **Provision Postgres.** Create a project at [supabase.com/dashboard](https://supabase.com/dashboard), then Project Settings -> Database -> Connect -> copy the **Transaction pooler** connection string (port 6543).
...
6. **Run migrations** against the deployed database: `DATABASE_URL=<supabase-pooled-url> npm run db:migrate`.
...
|---|---|---|
| `GEMINI_API_KEY` | drafting + transcription | From [Google AI Studio](https://aistudio.google.com/apikey). |
| `GEMINI_MODEL` | drafting/intake/angles/critique **and** transcription | Set to the most capable multimodal model on your account. No model name is hardcoded anywhere in the code - Gemini's multimodal models accept audio directly, so one model covers both. |
...
| `DATABASE_URL` | all persistence | Supabase Postgres pooled ("Transaction" mode) connection string. |
...
`lib/pricing.ts` holds a small maintained USD-per-million-token table for common Gemini models if you want to turn token counts into a dollar estimate yourself; it's not wired into any UI since there isn't one.

## Testing

```bash
npm test        # unit tests: deterministic checks, webhook guard (secret/allowlist/dedupe), callback/command parsing
npm run eval     # Appendix C: 10 end-to-end cases against the real Gemini API and a scratch Postgres
npm run smoke    # post-deploy: health check + webhook guard checks against a live deployment (set APP_URL first)
```

`npm run eval` and `npm run smoke` (via `npm run eval`'s `evals/localDb.ts`) boot a throwaway local Postgres automatically via `embedded-postgres` if `DATABASE_URL` isn't already set, so both work with zero setup. **`npm run eval` makes real calls to the Gemini API** (roughly 25-30 calls across the 10 cases) and will incur real usage cost.

This suite passed 10/10 against `gpt-4o` before the Gemini migration (see `docs/decisions.md`) - it has not yet been re-run against Gemini. Re-run it once `GEMINI_API_KEY`/`GEMINI_MODEL` are configured and update this line with the result before treating the pipeline as verified on the new backend.
...
- **`GEMINI_MODEL` and pricing table currency.** `lib/pricing.ts`'s per-model USD rates are a point-in-time snapshot and will drift; update them if you rely on the cost-estimate SQL/helper above.
- **Gemini's `responseSchema` conversion is unverified against the live API.** `lib/gemini.ts::zodToGeminiSchema` converts each stage's zod schema into Gemini's OpenAPI-subset schema dialect (uppercased types, `nullable: true` instead of a null union, stringified `minItems`/`maxItems`). This is unit-tested for shape (`lib/__tests__/gemini-schema.test.ts`) but not yet confirmed against a real Gemini response - do that first, per the build plan's own instruction to verify structured output against the live API before trusting a stage.
...
/lib
  ...
  gemini.ts  corpus.ts  skill.ts  pricing.ts
